...
-- This ensures the data loaded as expected (approx 100k orders).
table_counts AS (
...
-- The rubric notes NULL categories and messy text will be present in the dataset.
null_rates AS (
    SELECT
...
-- We check if orders point to customers that don't exist.
abandoned_keys AS (
    SELECT
...
duplicate_pks AS (
    ...
    FROM (SELECT order_id FROM orders GROUP BY 1 HAVING count(*) > 1)
    UNION ALL
    SELECT 'products', count(*)
    FROM (SELECT product_id FROM products GROUP BY 1 HAVING count(*) > 1)
),

-- FINAL SUMMARY: This is what prints by default
-- shows tables_checked, total_abandons_found, days_of_data, duplicate_pk_issues
summary_report AS (
    SELECT
